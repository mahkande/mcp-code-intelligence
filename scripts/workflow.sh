...
echo "   uv run mcp-code-intelligence  - Test CLI locally"
...
echo "   mcp-code-intelligence init    - Test initialization"
echo ""
echo "🚀 Stage C: PyPI Publication"
echo "   ./scripts/publish.sh      - Publish to PyPI"
echo "   pip install mcp-code-intelligence --upgrade  - Test published version"
echo ""
echo "🔧 Quick Commands:"
...
if [ -f "pyproject.toml" ]; then
    CURRENT_VERSION=$(uv run python -c "from mcp_code_intelligence import __version__; print(__version__)" 2>/dev/null || echo "unknown")
    ...
    if uv pip show mcp-code-intelligence 2>/dev/null | grep -q "Editable install"; then
        echo "✅ Development mode active (editable install)"
    else
        echo "ℹ️  Not in development mode - run: uv pip install -e ."
    fi
else
    echo "❌ Not in project directory - navigate to mcp-code-intelligence root"
fi
