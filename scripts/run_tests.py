#!/usr/bin/env python3
"""Comprehensive test runner for MCP Code Intelligence."""
...
def run_unit_tests(test_pattern: str = None) -> list[dict[str, Any]]:
    ...
    try:
        ...
        cmd.extend(["--cov=src/mcp_code_intelligence", "--cov-report=term-missing"])
...
def run_linting() -> list[dict[str, Any]]:
    ...
    if "mypy" in available_tools:
        cmd = ["python3", "-m", "mypy", "src/mcp_code_intelligence/"]
...
def run_smoke_tests() -> list[dict[str, Any]]:
    ...
    # Test basic imports
    cmd = ["python3", "-c", "import mcp_code_intelligence; print('✅ Import successful')"]
    result = run_command(cmd, "Basic Import Test")
    results.append(result)

    # Test CLI help
    cmd = ["python3", "-m", "mcp_code_intelligence", "--help"]
...
def main():
    """Main test runner."""
    parser = argparse.ArgumentParser(description="Run MCP Code Intelligence tests")
    ...
    print("🧪 MCP Code Intelligence Test Suite")
    print("=" * 60)
