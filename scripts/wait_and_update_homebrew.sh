...
check_pypi_availability() {
    local version=$1
    local url="https://pypi.org/pypi/mcp-code-intelligence/$version/json"
...
while [ $attempt -le $MAX_ATTEMPTS ]; do
    ...
    if check_pypi_availability "$VERSION"; then
        ...
        if [ -z "$HOMEBREW_TAP_TOKEN" ]; then
            ...
            echo "   cd /path/to/homebrew-mcp-code-intelligence"
            echo "   # Edit Formula/mcp-code-intelligence.rb"
            echo "   # Update version and sha256"
            echo "   git add Formula/mcp-code-intelligence.rb"
...
echo "2. Check availability: https://pypi.org/project/mcp-code-intelligence/$VERSION/"
...
echo "   cd \$(mktemp -d)"
echo "   git clone https://github.com/bobmatnyc/homebrew-mcp-code-intelligence.git"
echo "   cd homebrew-mcp-code-intelligence"
echo ""
echo "   # Get SHA256 from PyPI"
echo "   PYPI_SHA256=\$(curl -s https://pypi.org/pypi/mcp-code-intelligence/$VERSION/json | python3 -c \"import sys, json; data = json.load(sys.stdin); sdist = [r for r in data['urls'] if r['packagetype'] == 'sdist'][0]; print(sdist['digests']['sha256'])\")"
echo ""
echo "   # Update formula"
echo "   sed -i '' \"s/version \\\"[^\\\"]*\\\"/version \\\"$VERSION\\\"/g\" Formula/mcp-code-intelligence.rb"
echo "   sed -i '' \"s/sha256 \\\"[^\\\"]*\\\"/sha256 \\\"\$PYPI_SHA256\\\"/g\" Formula/mcp-code-intelligence.rb"
echo ""
