...
Automates the process of updating the Homebrew formula for mcp-code-intelligence:
1. Fetches latest version and SHA256 from PyPI
2. Clones/updates the homebrew-mcp-code-intelligence tap repository
...
Environment Variables:
    ...
    HOMEBREW_TAP_REPO: URL of tap repository (default: bobmatnyc/homebrew-mcp-code-intelligence)
...
class HomebrewFormulaUpdater:
    """Manages updates to Homebrew formula for mcp-code-intelligence."""
    ...
    def __init__(
        ...
        self.tap_repo_url = tap_repo_url or os.getenv(
            "HOMEBREW_TAP_REPO",
            "https://github.com/bobmatnyc/homebrew-mcp-code-intelligence.git",
        )
        ...
        if tap_repo_path:
            ...
            self.tap_repo_path = (
                Path.home() / ".homebrew_tap_update" / "homebrew-mcp-code-intelligence"
            )
        ...
        self.formula_name = "mcp-code-intelligence.rb"
    ...
    def fetch_pypi_info(self, version: str | None = None) -> PackageInfo:
        ...
        if version:
            url = f"https://pypi.org/pypi/mcp-code-intelligence/{version}/json"
        else:
            url = "https://pypi.org/pypi/mcp-code-intelligence/json"

        try:
            self.log(f"Requesting: {url}", "debug")

            request = Request(url)
            request.add_header("User-Agent", "mcp-code-intelligence-formula-updater/1.0")
    ...
    def update_formula(self, package_info: PackageInfo) -> Path:
        ...
        if version_match:
            ...
            # Extract version from URL (e.g., mcp_code_intelligence-0.15.16.tar.gz)
            url_match = re.search(
                r"mcp_code_intelligence-([0-9]+\.[0-9]+\.[0-9]+)\.tar\.gz", content
            )
            if url_match:
    ...
    def run(self, version: str | None = None) -> None:
        ...
        try:
            # Header
            print(f"\n{BOLD}{BLUE}{'=' * 60}{RESET}")
            print(f"{BOLD}{BLUE}Homebrew Formula Updater for mcp-code-intelligence{RESET}")
            ...
            if not self.dry_run:
                self.log(
                    "Users can now install with: brew install bobmatnyc/mcp-code-intelligence/mcp-code-intelligence",
                    "info",
                )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Update Homebrew formula for mcp-code-intelligence",
...
Exit Codes:
  ...
  5: Authentication error
    ...
    parser.add_argument(
        ...
        type=Path,
        metavar="PATH",
        help="Local path to tap repository (default: ~/.homebrew_tap_update/homebrew-mcp-code-intelligence)",
    )

    parser.add_argument(
        "--tap-repo-url",
        metavar="URL",
        help="URL of tap repository (default: from HOMEBREW_TAP_REPO or bobmatnyc/homebrew-mcp-code-intelligence)",
    )
