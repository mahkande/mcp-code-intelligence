#!/bin/bash
# Full Release Script for mcp-code-intelligence
...
import sys
sys.path.insert(0, '$PROJECT_DIR/src')
from mcp_code_intelligence import __build__
print(__build__)
"
}
...
echo ""
echo -e "${BOLD}${BLUE}============================================${NC}"
echo -e "${BOLD}${BLUE}  mcp-code-intelligence Full Release Script     ${NC}"
echo -e "${BOLD}${BLUE}============================================${NC}"
echo ""
...
if $DRY_RUN; then
    ...
    echo -e "${GREEN}✓ Published to PyPI: https://pypi.org/project/mcp-code-intelligence/$CURRENT_VERSION/${NC}"
fi
...
if $DRY_RUN; then
    ...
    while [ $ELAPSED -lt $MAX_WAIT ]; do
        if curl -s -f -o /dev/null "https://pypi.org/pypi/mcp-code-intelligence/$CURRENT_VERSION/json"; then
...
if ! $SKIP_PIPX; then
    ...
    if $DRY_RUN; then
        echo -e "${YELLOW}Would run: pipx upgrade mcp-code-intelligence --force${NC}"
    else
        echo "Upgrading local pipx installation..."
        pipx upgrade mcp-code-intelligence --force || {
            echo -e "${YELLOW}⚠ pipx upgrade failed, trying fresh install...${NC}"
            pipx install mcp-code-intelligence --force || {
                echo -e "${YELLOW}⚠ pipx install also failed, continuing...${NC}"
            }
        }

        # Verify installation
        INSTALLED_VERSION=$(mcp-code-intelligence --version 2>/dev/null | grep -oE '[0-9]+\.[0-9]+\.[0-9]+' | head -1 || echo "unknown")
...
echo "  PyPI:    https://pypi.org/project/mcp-code-intelligence/$CURRENT_VERSION/"
echo "  GitHub:  https://github.com/bobmatnyc/mcp-code-intelligence/releases/tag/v$CURRENT_VERSION"
if [ -n "$HOMEBREW_TAP_TOKEN" ]; then
    echo "  Homebrew: brew install bobmatnyc/mcp-code-intelligence/mcp-code-intelligence"
fi
echo ""
echo -e "${CYAN}Install/Upgrade Commands:${NC}"
echo "  pip:     pip install mcp-code-intelligence==$CURRENT_VERSION"
echo "  pipx:    pipx upgrade mcp-code-intelligence --force"
echo "  brew:    brew upgrade mcp-code-intelligence"
echo ""
