"""Index command for MCP Code Intelligence CLI."""
...
def main(
    ...
    workers: int | None = typer.Option(
        None,
        "--workers",
        help="Maximum number of worker processes for parsing",
        rich_help_panel="⚡ Performance",
    ),
    throttle: float = typer.Option(
        0.0,
        "--throttle",
        help="Delay in seconds between batches to reduce CPU load",
        rich_help_panel="⚡ Performance",
    ),
    max_size: int = typer.Option(
        1024,
        "--max-size",
        help="Maximum file size in KB to index (default: 1MB)",
        rich_help_panel="⚡ Performance",
    ),
    important_only: bool = typer.Option(
        False,
        "--important-only",
        help="Only index core source files (skip documentation, config, etc.)",
        rich_help_panel="⚡ Performance",
    ),
) -> None:
    ...
    [green]Force reindex without analysis:[/green]
        $ mcp-code-intelligence index --force --no-analyze

    [green]Custom file extensions:[/green]
        $ mcp-code-intelligence index --extensions .py,.js,.ts,.md

    [bold cyan]Advanced Usage:[/bold cyan]

    [green]Watch mode (experimental):[/green]
        $ mcp-code-intelligence index --watch

    [green]Full reindex (no incremental):[/green]
        $ mcp-code-intelligence index --full

    [green]Optimize for large projects:[/green]
        $ mcp-code-intelligence index --batch-size 64
...
async def run_indexing(
    ...
    if extensions:
        ...
        # Update config with other overrides
        config = config.model_copy(update={
            "max_workers": workers,
            "throttle_delay": throttle,
            "max_file_size_kb": max_size,
...
async def _run_batch_indexing(
    ...
    if show_progress:
        ...
        if total_files == 0:
            ...
            if failed_count > 0:
                ...
                error_log_path = (
                    indexer.project_root / ".mcp-code-intelligence" / "indexing_errors.log"
                )
    ...
    if indexed_count > 0:
        ...
        config_dir = indexer.project_root / ".mcp-code-intelligence"
        ...
        if llm_configured:
            ...
            chat_hint = f"[cyan]mcp-code-intelligence chat 'question'[/cyan] - Ask AI about your code [green](✓ {provider})[/green]"
        else:
            chat_hint = "[cyan]mcp-code-intelligence chat 'question'[/cyan] - Ask AI about your code [dim](requires API key)[/dim]"

        steps = [
            "[cyan]mcp-code-intelligence search 'your query'[/cyan] - Try semantic search",
            chat_hint,
            "[cyan]mcp-code-intelligence status[/cyan] - View detailed statistics",
        ]
        ...
        print_tip(
            "Check configured extensions with [cyan]mcp-code-intelligence status[/cyan]"
        )
...
def reindex_file(
    ...
    Examples:
        mcp-code-intelligence index reindex                     # Reindex entire project
        mcp-code-intelligence index reindex --all               # Explicitly reindex entire project
        mcp-code-intelligence index reindex src/main.py         # Reindex specific file
        mcp-code-intelligence index reindex --all --force       # Reindex entire project without confirmation
    """
    try:
...
def watch_cmd(
    ...
    Examples:
        mcp-code-intelligence index watch
        mcp-code-intelligence index watch /path/to/project
    """
...
def health_cmd(
    ...
    Examples:
        mcp-code-intelligence index health
        mcp-code-intelligence index health --repair
    """
...
def cancel_cmd(
    ...
    Examples:
        ...
        mcp-code-intelligence index cancel --force
    """
    try:
        project_root = ctx.obj.get("project_root") or Path.cwd()
...
def _spawn_background_relationships(project_root: Path) -> None:
    ...
    try:
        ...
        print_info(
            f"Log file: {project_root / '.mcp-code-intelligence' / 'relationships_background.log'}"
        )
