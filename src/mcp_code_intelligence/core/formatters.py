...
def format_locations(locations: Iterable[Any]) -> List[TextContent]:
    ...
    if not out:
        out.append(TextContent(type="text", text="Aradığınız sembol bu dosyada bulunamadı."))
...
def format_definition_response(resp: Any) -> List[TextContent]:
    ...
    if resp is None:
        return [TextContent(type="text", text="Aradığınız sembol bu dosyada bulunamadı.")]
...
def format_references_response(resp: Any) -> List[TextContent]:
    ...
    if isinstance(payload, list):
        ...
        if total > 20:
            summary = TextContent(type="text", text=f"Toplam {total} referans bulundu. İlk 5 gösteriliyor. Daha fazlasını ister misiniz? (evet/hayır)")
...
def format_lsp_error(e: Exception) -> List[TextContent]:
    return [TextContent(type="text", text=f"LSP hata: {str(e)}")]
