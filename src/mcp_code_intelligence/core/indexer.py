...
class SemanticIndexer:
    ...
    async def index_project(
        ...
        for i in range(0, len(files_to_index), self.batch_size):
            ...
            # ... (batch processing logic remains same)
        ...
        if not skip_relationships and indexed_count > 0:
            try:
                ...
                if len(all_chunks) > 0:
                    ...
                    logger.info(
                        "  Use 'mcp-code-intelligence index relationships' to compute now or wait for background task"
                    )
                else:
    ...
    async def _process_file_batch(
        ...
        for file_path, chunks, error in parse_results:
            ...
            if chunks:
                ...
                # Filter out low-signal chunks (getters, setters, tiny chunks)
                chunks_with_hierarchy = [
                    c for c in chunks_with_hierarchy if not self.rag_guard.is_low_signal_chunk(c)
                ]

                if not chunks_with_hierarchy:
                    logger.debug(f"All chunks from {file_path} filtered as low-signal")
    ...
    def _load_index_metadata(self) -> dict[str, dict[str, Any]]:
        ...
        Returns:
            Dictionary mapping file paths to {mtime, hash}
        """
    ...
    def _save_index_metadata(self, metadata: dict[str, dict[str, Any]]) -> None:
        ...
        Args:
            metadata: Dictionary mapping file paths to metadata dicts
        """
        try:
    ...
    def _needs_reindexing(self, file_path: Path, metadata: dict[str, dict[str, Any]]) -> bool:
        ...
        try:
            ...
            # If mtime is the same, file likely hasn't changed
            if current_mtime <= stored_mtime:
                return False

            # If mtime changed, check hash for definitive answer
    ...
    def _should_index_file(
        ...
        try:
            # Reverted to more generous 10MB default to avoid missing important files
        ...
        # SMARTER OPTIMIZATION: Check if file is minified or data-heavy text
        # This is more effective than size limits for preventing freezes
    ...
    def _should_ignore_path(
        ...
        """
        try:
            # 0. CRITICAL PERFORMANCE GUARD: Explicitly block common heavy directories
            # We check the path name itself against a blacklist regardless of relative path calculation
            # This handles cases where relative_to might fail or gitignore has issues
            ...
            # 5. Check RAG Guard (Test/Mock/Config filters)
    ...
    async def _parse_file(self, file_path: Path) -> list[CodeChunk]:
        ...
        Args:
            file_path: Path to the file to parse
        """
        # Read content to check if it should be indexed (Secret/Generated check)
        try:
            content = file_path.read_text(encoding="utf-8", errors="ignore")
            if not self.rag_guard.should_index_content(content):
                logger.debug(f"Content ignored by RAG Guard: {file_path}")
                return []
        except Exception as e:
            logger.warning(f"Failed to read file for RAG Guard check: {file_path}: {e}")
    ...
    async def index_files_with_progress(
        ...
        for i in range(0, len(files_to_index), self.batch_size):
            ...
            for file_path, result in zip(batch, parse_results, strict=True):
                ...
                if chunks:
                    # Granular Incremental Indexing: Compare with existing chunks (Lightweight)
                    try:
                        ...
                        if stale_ids:
                            await self.database.delete_chunks(stale_ids)
                            if len(stale_ids) > 0:
                                logger.debug(f"Deleted {len(stale_ids)} stale chunks from {file_path}")
