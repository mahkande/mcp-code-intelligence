...
function startMcpServer(projectRoot: string): ChildProcessWithoutNullStreams {
  // MCP server Python entrypoint
  const script = path.join(projectRoot, 'src', 'mcp_code_intelligence', 'mcp', 'server.py');
...
export function activate(context: vscode.ExtensionContext) {
  ...
  // Ensure rule files exist (auto-create if missing)
  ensureRuleFiles(projectRoot);
  // Start MCP server if not running
  if (!mcpProcess) {
    mcpProcess = startMcpServer(projectRoot);
  }
  // Her dosya kaydedildiğinde kural dosyalarını sessizce doğrula
